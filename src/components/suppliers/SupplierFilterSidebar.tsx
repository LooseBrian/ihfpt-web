...
export function SupplierFilterSidebar({
  ...
  onChange,
  resultCount,
}: SupplierFilterSidebarProps) {
  const totalActive = Object.values(filters).reduce(
    (sum, arr) => sum + arr.length,
    0
  );

  const updateFilter = (key: keyof SupplierFilters, id: string) => {
    ...
    <aside className="w-full lg:w-64 shrink-0">
      <div className="bg-white rounded-xl border shadow-sm p-4 sticky top-24">
        ...
        <div className="text-xs text-muted-foreground mb-4">
          共找到 <span className="font-semibold text-brand-700">{resultCount}</span> 家企业
        </div>
        ...
        <FilterSection
          title="企业经营类型"
          options={filterBusinessTypes}
          selected={filters.businessTypes}
          onToggle={(id) => updateFilter("businessTypes", id)}
        />
        <FilterSection
          title="核心资质"
          options={filterSupplierQualifications}
          selected={filters.qualifications}
          onToggle={(id) => updateFilter("qualifications", id)}
        />
        <FilterSection
          title="服务能力"
